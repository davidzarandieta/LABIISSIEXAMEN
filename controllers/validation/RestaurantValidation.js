...
// SOLUTION
const checkBusinessRuleOneResturantPromotedByOwner = async (ownerId, promocionalValue) => {
  let isBusinessRuleToBeBroken = false
  if (promocionalValue === 'true') {
    try {
      const promotedRestaurants = await Restaurant.findAll({ where: { userId: ownerId, promocional: true } })
